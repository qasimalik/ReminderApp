...
const AllRemindersScreen = ({ route }) => {
  const navigation = useNavigation();
  const isSchemaReady = useSchemaReady();
  const { getAllIncompleteReminders} = useReminderDAO();
  ...
  useEffect(() => {
    ...
    const loadReminders = async () => {
      try {
        const fetchedReminders = await getAllIncompleteReminders(id);
    ...
    loadReminders();
    return unsubscribe;
  }, [navigation, isSchemaReady, getAllIncompleteReminders, id]);

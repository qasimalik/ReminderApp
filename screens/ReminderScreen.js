...
const ReminderScreen = ({ route }) => {
  ...
  const { getIncompleteRemindersByListId} = useReminderDAO();
  ...
  useEffect(() => {
    if (!isSchemaReady || !id) return;

    const loadReminders = async () => {
      try {
        const fetchedReminders = await getIncompleteRemindersByListId(id);
        setReminders(fetchedReminders);
        // console.log("Reminders loaded successfully:", fetchedReminders);
      } catch (error) {
    ...
    loadReminders();
    return unsubscribe;
  }, [navigation, isSchemaReady, getIncompleteRemindersByListId, id]);
      ...
      <SafeAreaView style={styles.container}>
        ...
        <View style={styles.bottomButton}>
          <TouchableOpacity
            ...
            onPress={() => navigation.navigate("Add Reminder", { listId: id })}
          >
            <FontAwesome6 name="plus" size={14} color="orange" />
            <Text style={styles.reminderButtonText}> New Reminder</Text>
          </TouchableOpacity>
        </View>
      </SafeAreaView>
    </SafeAreaProvider>

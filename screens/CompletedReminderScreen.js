...
const CompletedReminderScreen = ({ route }) => {
  const navigation = useNavigation();
  const isSchemaReady = useSchemaReady();
  const { getCompletedReminders} = useReminderDAO();
  ...
  useEffect(() => {
    ...
    const loadReminders = async () => {
      try {
        const fetchedReminders = await getCompletedReminders();
    ...
    loadReminders();
    return unsubscribe;
  }, [navigation, isSchemaReady, getCompletedReminders]);
...
export default CompletedReminderScreen;
